...
$subscriptionTypeFilter = $_GET['subscription_type_id'] ?? '';
...
$logs = $usageLogModel->getAll([
    ...
    'action' => $actionFilter,
    'days' => $dateFilter,
    'subscription_type_id' => $subscriptionTypeFilter
]);
                    ...
                    <div class="card">
                        ...
                        <div class="card-body">
                            <form method="GET" class="row">
                                ...
                                <div class="col-md-2">
                                    <label for="subscription_type_id" class="form-label">Subscription Type</label>
                                    <select class="form-control select2" id="subscription_type_id" name="subscription_type_id">
                                        <option value="">All Types</option>
                                        <?php foreach ($subscriptionTypeModel->getAll(['is_active' => 1]) as $type): ?>
                                        <option value="<?php echo $type['id']; ?>" <?php echo $subscriptionTypeFilter == $type['id'] ? 'selected' : ''; ?>>
...
<script>
// Tablesort and AJAX search initialization
document.addEventListener('DOMContentLoaded', function() {
    var table = document.getElementById('usage-logs-table');
    if (table) {
        Tablesort(table);
        // Sort by date column (first column, descending)
        var th = table.querySelector('th');
        if (th) th.click(); // first click ascending
        if (th) th.click(); // second click descending
    }
    // AJAX search
    ...
    if (searchInput) {
        searchInput.addEventListener('input', function() {
            ...
            fetch('usage-logs-search.php?q=' + encodeURIComponent(query))
                .then(response => response.text())
                .then(html => {
                    var tbody = document.querySelector('#usage-logs-table tbody');
                    if (tbody) tbody.innerHTML = html;
                    // Re-init sort after AJAX
                    Tablesort(table);
                    var th = table.querySelector('th');
                    if (th) th.click();
                    if (th) th.click();
