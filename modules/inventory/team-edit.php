...
$stmt = $db->prepare("
    SELECT COUNT(*) as assignments_count,
           COALESCE(SUM(COALESCE(i.monthly_cost, i.annual_cost, 0)), 0) as total_cost
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {
    ...
    if ($_POST['action'] === 'add_member') {
        // Debug logging
        error_log('Processing add_member action');
        
        $userId = (int)$_POST['user_id'];
        $isLead = isset($_POST['is_lead']) && $_POST['is_lead'] == '1' ? 1 : 0;
        
        error_log('Parsed values - User ID: ' . $userId . ', Is Lead: ' . $isLead);
        ...
        if ($isLead) {
            error_log('Removing lead status from other members...');
            $stmt = $db->prepare("UPDATE inventory_team_members SET is_lead = 0 WHERE team_id = ?");
            $stmt->execute([$teamId]);
        }
        
        // Add new member
        error_log('Adding new member to team...');
        $stmt = $db->prepare("INSERT INTO inventory_team_members (team_id, user_id, is_lead, created_by, created_at) VALUES (?, ?, ?, ?, NOW())");
        
        if ($stmt->execute([$teamId, $userId, $isLead, $_SESSION['user_id']])) {
            error_log('SUCCESS: Member added successfully');
            echo json_encode(['success' => true, 'message' => 'Member added successfully']);
        } else {
            error_log('ERROR: Failed to add member - ' . implode(', ', $stmt->errorInfo()));
            echo json_encode(['success' => false, 'message' => 'Failed to add member: ' . implode(', ', $stmt->errorInfo())]);
        }
        exit;
    }
                                    ...
                                    <div class="col-md-6">
                                        <div class="form-group">
                                            ...
                                            <select class="form-control" id="status" name="status">
                                                <option value="active" <?php echo ($teamData['is_active'] == 1) ? 'selected' : ''; ?>>Active</option>
                                                <option value="inactive" <?php echo ($teamData['is_active'] == 0) ? 'selected' : ''; ?>>Inactive</option>
                                            </select>
                                        </div>
                                    </div>
                                </div>
            ...
            <form id="addMemberForm" onsubmit="return false;">`
                <div class="modal-body">
                    <div class="form-group">
                        ...
                        <select class="form-control" id="new_member_id" name="user_id" required>
...
$(document).ready(function() {
    console.log('Document ready - team edit page loaded');
    ...
    console.log('Select2 initialized for team manager');

    // Modal event handlers
    $('#addMemberModal').on('shown.bs.modal', function() {
        console.log('Modal shown');
        // Modal açıldığında hiçbir özel işlem yapmaya gerek yok
    });
    ...
    $('#addMemberModal').on('hidden.bs.modal', function() {
        ...
        // Modal backdrop temizle
        $('.modal-backdrop').remove();
        $('body').removeClass('modal-open').css('padding-right', '');
    });

    // Handle cancel button and close button
    $(document).on('click', '#addMemberModal [data-bs-dismiss="modal"]', function(e) {
        e.preventDefault();
        e.stopPropagation();
    ...
    $('#addMemberModal').on('click', function(e) {
        if (e.target === this) {
            $(this).modal('hide');
        }
    });
    ...
    $('[data-bs-toggle="modal"][data-bs-target="#addMemberModal"]').on('click', function(e) {
        e.preventDefault();
    ...
    // Add member form submission - prevent any default form submission
    $(document).on('submit', '#addMemberForm', function(e) {
        ...
        // Get form data
        var userId = $('#new_member_id').val();
        var isLead = $('#make_leader').is(':checked') ? 1 : 0;
